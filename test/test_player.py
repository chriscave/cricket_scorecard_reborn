...
from scorecard import Player


class TestPlayer(unittest.TestCase):
    def test_add_ball_bowler(self):
        player = Player("a")
        player.bowler_add_ball("1", 1, 2)
        actual = player.bowl_inns
        expected = {1: ["DNB", "DNB", "1", "DNB", "DNB", "DNB"]}
        self.assertEqual(actual, expected)

    def test_add_ball_batter(self):
        player = Player("a")
        player.batter_add_ball("0", 1, 2)
        actual = player.bat_inns
        expected = {1: ["DNB", "DNB", "0", "DNB", "DNB", "DNB"]}
        self.assertEqual(actual, expected)
